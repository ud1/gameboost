									 ...
									 const unsigned short maxIndex)
{
	...
	{	
		...
		else 
		{
			...
			{
				ERROR_LOG("BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences");
			}
			else
			{
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			ERROR_LOG("GetNextIndex: Triangle doesn't have all of its vertices");
			ERROR_LOG("GetNextIndex: Duplicate triangle probably got us derailed");
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
