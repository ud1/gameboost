...
//--------------------------------------------------------------------------------------
ListBox::ListBox( Dialog *pDialog ) :
m_ScrollBar( pDialog )
{
	m_Type = MYUT_CONTROL_TYPE::LISTBOX;
	m_pDialog = pDialog;
...
void ListBox::UpdateRects()
{
	...
	FontNode* pFontNode = m_pDialog->GetManager()->GetFontNode( m_Elements.GetAt( 0 )->iFont );
...
bool ListBox::MsgProc( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	if( WM_CAPTURECHANGED == uMsg )
	{
		...
		if( (HWND)lParam !=  m_pDialog->getHwnd() )
			m_bDrag = false;
	}
...
//--------------------------------------------------------------------------------------
void ListBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	if( m_bVisible == false )
		return;
	...
	pElement->FontColor.Blend( MYUT_CONTROL_STATE::NORMAL, fElapsedTime );

	Element* pSelElement = m_Elements.GetAt( 1 );
	pSelElement->TextureColor.Blend( MYUT_CONTROL_STATE::NORMAL, fElapsedTime );
	pSelElement->FontColor.Blend( MYUT_CONTROL_STATE::NORMAL, fElapsedTime );
