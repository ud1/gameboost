...
//--------------------------------------------------------------------------------------
EditBox::EditBox( Dialog *pDialog )
{
	m_Type = MYUT_CONTROL_TYPE::EDITBOX;
	m_pDialog = pDialog;
...
#pragma message("ks777: WARN; COMMENTED CODE !!! "  __FILE__)
	//m_dfLastBlink = MYUTGetGlobalTimer()->GetAbsoluteTime();
	assert(false);
...
//--------------------------------------------------------------------------------------
void EditBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
	if( m_bVisible == false )
		return;
...
#pragma message("ks777: WARN; COMMENTED CODE !!! "  __FILE__)
	/*******************
	if( MYUTGetGlobalTimer()->GetAbsoluteTime() - m_dfLastBlink >= m_dfBlink )
	{
