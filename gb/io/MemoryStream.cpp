...
{
    if(file || (mode & eStreamAccess::APPEND) || (mode & eStreamAccess::TRUNCATE))
        return file;
    file_size = _size;
    offset = 0;
    stream_access = mode;
...
{
    if(!file || !buffer || !size || !(stream_access & eStreamAccess::WRITE))
        return 0;
...
{
    if(!file || !buffer || !size || !(stream_access & eStreamAccess::READ))
        return 0;
